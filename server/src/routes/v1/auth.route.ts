...
export const authRouter = () => {
    ...
    router.post('/refreshToken', async (req, res, next) => {
        try {
            ...
            const payload = <JwtPayload>jwt.verify(refreshToken, <string>process.env.JWT_REFRESH_SECRET);
            ...
            const user = await usersServices.findUserById(<string>payload.userId);
            if (!user) {
                res.status(401);
                throw new Error('Unauthorized');
            }
